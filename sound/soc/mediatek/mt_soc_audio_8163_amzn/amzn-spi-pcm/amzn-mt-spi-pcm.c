...
#define FPGA_OVERRUN_METRIC_MAX 10
...
struct amzn_spi_priv {
	...
	bool fpga_overruns_persistent;
	unsigned long last_duration;
	unsigned long overrun_duration;
	unsigned long slept_duration;
	unsigned long spi_duration;
#endif
...
static void kernel_overrun_metrics(struct work_struct *work)
{
	...
	if (spi_data.kernel_overruns) {
		snprintf(buf, sizeof(buf),
			"%s:%s,MetricKey=KernelOverrun;SY,IntValue=%zu;IN,StrValue=amzn-mt-spi-pcm;SY:%s",
			AUDIO_MINERVA_METRIC_HEADER,
			AUDIO_MINERVA_METRIC_PREDEFINED_FIELDS,
			spi_data.kernel_overruns, MINERVA_METRIC_TAIL);
...
static void fpga_overrun_metrics(struct work_struct *work)
{
	...
	size_t overrun_metric_count;

	if (!spi_data.fpga_overruns_persistent) {
		metric_key = "FPGAOverrun";
		logging_string = "FPGA_OVERRUN";
		overrun_metric_count = spi_data.fpga_overruns;
	} else {
		metric_key = "FPGAOverrunPersistent";
		logging_string = "PERSISTENT_FPGA_OVERRUN";
		overrun_metric_count = 1;
	}

	/* No need of logging 0 overruns */
	if (overrun_metric_count) {
		snprintf(buf, sizeof(buf),
			"%s:%s,MetricKey=%s;SY,IntValue=%zu;IN,StrValue=amzn-mt-spi-pcm;SY:%s",
			AUDIO_MINERVA_METRIC_HEADER,
			AUDIO_MINERVA_METRIC_PREDEFINED_FIELDS,
			metric_key,
			overrun_metric_count, MINERVA_METRIC_TAIL);

		log_to_metrics(ANDROID_LOG_WARN, "AudioRecordOverrun", buf);
		/* move logging in another kworker thread to avoid blocking spi read thread */
		pr_err("%s: %s prev_cycle_us=%lu curr_cycle_us=%lu slept_us=%lu spi_trx_us=%lu overruns=%zu\n",
			__func__, logging_string, spi_data.last_duration, spi_data.overrun_duration,
			spi_data.slept_duration, spi_data.spi_duration,
			overrun_metric_count);
...
static void spi_data_read(struct work_struct *work)
{
	...
	unsigned long time_diff_usec = 0, irq_flags;
	...
	uint32_t fpga_metric_count = 0;
	bool need_metric = false;
	...
	spi_priv_data->fpga_overruns_persistent = false;
	...
	while (get_run_thread()) {
		...
		if (rx_df->dsf.overrun == 1) {
			...
			spi_priv_data->overrun_duration = ktime_diff(&cur_ktime, &sleep_ktime);
			/* Time spent in last sleep */
			spi_priv_data->slept_duration = ktime_diff(&prev_ktime, &sleep_ktime);
			/* Time spent in last spi transaction */
			spi_priv_data->spi_duration =  ktime_diff(&cur_ktime, &prev_ktime);

			spi_priv_data->last_duration = time_diff_usec;

			if (fpga_metric_count < FPGA_OVERRUN_METRIC_MAX) {
				++spi_priv_data->fpga_overruns;
				++fpga_metric_count;
				need_metric = true;
			} else if ((fpga_metric_count == FPGA_OVERRUN_METRIC_MAX) &&
					!spi_priv_data->fpga_overruns_persistent) {
				// do not increment fpga_overrun here, persistent metric is val 1
				spi_priv_data->fpga_overruns_persistent = true;
				need_metric = true;
			} else {
				// do not increment fpga_overrun here, persistent metric was sent already
				need_metric = false;
			}
#ifdef CONFIG_AMAZON_METRICS_LOG
