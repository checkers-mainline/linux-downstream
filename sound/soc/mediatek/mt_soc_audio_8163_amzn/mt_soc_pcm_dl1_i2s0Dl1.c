	...
	struct snd_pcm_substream *substream)
{
	...
	if (GetMemoryPathEnable(Soc_Aud_Digital_Block_MEM_DL1) == true) {
		...
		if (Afe_Block->u4DataRemained < Afe_consumed_bytes ||
			...
			pr_err("%s underflow A :u4DataRemained 0x%x-> Afe_consumed_bytes 0x%x -> u4BufferSize 0x%x\n", __func__,Afe_Block->u4DataRemained, Afe_consumed_bytes, Afe_Block->u4BufferSize);
			pr_err("%s underflow B :HW_memory_index 0x%x-> u4DMAReadIdx 0x%x \n", __func__,HW_memory_index, Afe_Block->u4DMAReadIdx);
			mtkpcm_underflow++;
			Afe_Block->u4DataRemained = 0;
...
static int mtk_I2S0dl1_probe(struct platform_device *pdev)
{
	...
	proc_create("mtkpcm_stats", 0, NULL, &mtk_pcm_proc_fops);
