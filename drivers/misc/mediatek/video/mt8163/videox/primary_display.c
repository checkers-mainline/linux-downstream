...
int primary_display_get_avg_fps(unsigned int measure_time)
{
	unsigned int fps = 0;
	unsigned int decimal = 0;
	unsigned int cnt = 0;
	unsigned long long start_time = ktime_to_ns(ktime_get());
	unsigned long long spend_time = 0;
	unsigned long long timens =
		(unsigned long long)measure_time * 1000 * 1000 * 1000;

	while (1) {
		dpmgr_wait_event_timeout(pgc->dpmgr_handle,
			DISP_PATH_EVENT_FRAME_START, HZ * 1);
		cnt++;
		if (ktime_to_ns(ktime_get()) - start_time > timens)
			break;
	}
	spend_time = ktime_to_ns(ktime_get()) - start_time;
	do_div(spend_time , 1000 * cnt);
	fps = 1000 * 1000 / (int)spend_time;
	decimal = (100 * 1000 * 1000 / (int)spend_time) % 100;
	DISPMSG("############ vsync_time=%d(us), average fps=%d.%02d ##########\n",
		(int)spend_time, fps, decimal);
	return 0;
}
