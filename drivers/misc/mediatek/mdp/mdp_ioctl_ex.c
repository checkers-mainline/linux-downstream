			...
			struct cmdqRecStruct *handle)
{
	...
	case CMDQ_MOP_READ:
	{
		...
		if (!reg_addr || !dram_addr)
			return -EINVAL;
		status = cmdqRecBackupRegisterToSlot(handle,
	...
	case CMDQ_MOP_POLL:
		...
		if (!reg_addr)
			return -EINVAL;
		status = cmdqRecPoll(handle, reg_addr,
					meta->value, meta->mask);
		break;
	case CMDQ_MOP_WAIT:
		status = cmdqRecWait(handle, meta->event);
		break;
	case CMDQ_MOP_WAIT_NO_CLEAR:
		status = cmdqRecWaitNoClear(handle, meta->event);
		break;
	case CMDQ_MOP_CLEAR:
		status = cmdqRecClearEventToken(handle, meta->event);
		break;
	case CMDQ_MOP_SET:
		status = cmdqRecSetEventToken(handle, meta->event);
		break;
		...
		if (metas[i].op == CMDQ_MOP_READ) {
			...
			if (rb_slot[slot_id].ref_cnt == MAX_REF_COUNT) {
				...
				return -EINVAL;
                        ...
                        /*store slot_id to handle->slot_ids[] array. and ref_cnt++ */
				...
				struct cmdqRecStruct *handle)
{
	...
	for (i = 0; i < count; i++) {
		...
		cmdqRecBackupRegisterToSlot(handle, handle->reg_values_pa,
					i, reg_addr);
	}
