...
static ssize_t power_adc_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct iio_channel *channel;
	int val,ret;

	channel = iio_channel_get(dev, "power-channel");
	if (IS_ERR(channel)) {
		ret = PTR_ERR(channel);
...
static DEVICE_ATTR(power_adc, 0664, power_adc_show, NULL);
...
static int pmic_mt6323_probe(struct platform_device *dev)
{
	...
	device_create_file(&(dev->dev), &dev_attr_power_adc);
	return 0;
