...
{
	return cmdqRecWrite(handle, addr, argument, mask);
}
	...
	u32 write_reg, u32 from_reg)
{
	...
	do {
		status = cmdqRecReadToDataRegister(handle, from_reg,
			CMDQ_DATA_REG_JPEG);
		if (status < 0)
			break;

		status = cmdqRecWriteFromDataRegister(handle,
...
s32 cmdq_mdp_handle_create(struct cmdqRecStruct **handle_out)
{
	...
	status = cmdqRecCreate(CMDQ_SCENARIO_USER_MDP, &handle);
	if (status < 0) {
			...
			struct cmdqRecStruct *handle)
{
	...
	/* set secure data */
	if (!secData || !secData->isSecure)
		return 0;

	CMDQ_MSG("%s start:%d, %d\n", __func__,
		secData->isSecure, secData->addrMetadataCount);
	if (!secData->addrMetadataCount ||
		(secData->addrMetadataCount > MDP_MAX_METADATA_COUNT_SIZE)) {
		CMDQ_ERR(
			"[secData]mismatch isSecure %d and addrMetadataCount %d\n",
			secData->isSecure,
	...
	cmdqRecSetSecure(handle, secData->isSecure);
...
{
	return cmdqRecGetInstructionCount(handle);
}
