...
static uint32_t *cmdq_core_get_pc(const struct TaskStruct *pTask,
	uint32_t thread,
	uint32_t insts[4]);
static int32_t cmdq_core_get_event(enum CMDQ_EVENT_ENUM event);
...
void cmdq_core_init_DTS_data(void)
{
	...
	for (i = 0; i < CMDQ_SYNC_TOKEN_MAX; i++) {
		if (i <= CMDQ_MAX_HW_EVENT_COUNT) {
			/* GCE HW event */
			gCmdqDtsData.eventTable[i] = CMDQ_SYNC_TOKEN_INVALID
						     - 1 - i;
		} else {
			/* GCE SW event */
...
int32_t cmdq_core_get_event_value(enum CMDQ_EVENT_ENUM event)
{
	...
	return cmdq_core_get_event(event);
}
...
int32_t cmdq_core_get_event(enum CMDQ_EVENT_ENUM event)
{
#undef DECLARE_CMDQ_EVENT
#define DECLARE_CMDQ_EVENT(name, val) case val:return val;
