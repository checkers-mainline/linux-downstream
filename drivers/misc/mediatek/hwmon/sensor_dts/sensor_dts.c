...
struct alsps_hw *get_alsps_dts_func(const char *name, struct alsps_hw *hw)
{
	...
	u32 is_supported_two_point_als_cali[] = {0};
	struct device_node *node = NULL;
	...
	if (node) {
		...
		ret = of_property_read_u32_array(node, "is_supported_two_point_als_cali", is_supported_two_point_als_cali,
			ARRAY_SIZE(is_supported_two_point_als_cali));
		if (ret == 0)
			hw->is_supported_two_point_als_cali = is_supported_two_point_als_cali[0];
		else
			hw->is_supported_two_point_als_cali = 0;
