...
static void hs_slew_rate_cal(void)
{
	...
	if (node == NULL)
		DBG(0, "get USB node failed, set default hs_slew_rate\n");
	else {
		ret = of_property_read_u32(node, "hs_slew_rate", &hs_slew_rate);
		if (ret < 0 || hs_slew_rate > 7) {
			DBG(0, "get hs_slew_rate fail, ret(%d), value(%d)\n",
				ret, hs_slew_rate);
			hs_slew_rate = 6;
		}
	}
#endif
	USBPHY_WRITE8(0x15, (hs_slew_rate & 0x7) << 4);
