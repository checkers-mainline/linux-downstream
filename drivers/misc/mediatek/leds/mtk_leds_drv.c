...
extern char *leds_name[MT65XX_LED_TYPE_TOTAL];
...
static int mt65xx_leds_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < MT65XX_LED_TYPE_TOTAL; i++) {
		...
		if (strcmp(g_leds_data[i]->cdev.name,leds_name[MT65XX_LED_TYPE_LCD]) == 0) {
			g_leds_data[i]->cdev.brightness = cust_led_list[i].def_brightness;
		}
