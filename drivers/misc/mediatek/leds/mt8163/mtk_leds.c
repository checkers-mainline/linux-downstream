...
struct cust_mt65xx_led *get_cust_led_dtsi(void)
{
	...
	int i, ret;
	int mode, data, def_brightness;
	int pwm_config[5] = { 0 };
#if defined(CONFIG_ENABLE_BACKLIGHT_FACTOR)
    int normalize_param[2] = { 0 };
#endif

#if defined(CONFIG_CUST_BRI_SUPPORT)
	int ntable;
#endif
	...
	if (pled_dtsi == NULL) {
		...
		for (i = 0; i < MT65XX_LED_TYPE_TOTAL; i++) {
			...
			if (!led_node) {
				...
				ret =
					of_property_read_u32(led_node, "led_def_brightness",
						&def_brightness);
				if ((!ret) && (def_brightness <= LED_FULL) && (def_brightness > 0)) {
					pled_dtsi[i].def_brightness = def_brightness;
					LEDS_DEBUG
						("The %s's led def_brightness is : %d\n",
						pled_dtsi[i].name,
						pled_dtsi[i].def_brightness);
				} else {
					LEDS_DEBUG
						("led dts can not get led def_brightness");
					pled_dtsi[i].def_brightness = 0;
				}
...
#if defined(CONFIG_CUST_BRI_SUPPORT)
				mt_mt65xx_led_set_cust(cust_level);
#else
