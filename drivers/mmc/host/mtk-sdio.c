...
int execute_online_tuning(struct msdc_host *host, u8 *res)
{
	...
	do {
		...
		if (score < 10 || autok_check_scan_res64(RawData64, pBdInfo) != 0) {
	...
	do {
		...
		if (score < 10 || autok_check_scan_res64(RawData64, pBdInfo) != 0) {
			ATK_ERR("[AUTOK]Tune read DAT Fail\n");
			host->autok_error = -1;
			return -1;
		}
		#if 0
...
void msdc_module_reset(struct msdc_host *host)
{
	u32 val;

	sdr_clr_bits(host->base + MSDC_CFG, MSDC_CFG_MODE);
	/* Run MSDC infra module reset */
	val = readl(host->infra_reset);
	pr_debug("init 0x10001120: 0x%x, MSDC_CFG: 0x%x\n",
		 val, readl(host->base + MSDC_CFG));
	writel(0x1 << host->module_reset_bit, host->infra_reset);
	val = readl(host->infra_reset);
	udelay(1);
	pr_debug("msdc module resetting 0x10001120: 0x%x\n", val);
	writel(0x1 << host->module_reset_bit, host->infra_reset + 0x04);
	udelay(1);
	val = readl(host->infra_reset);
	pr_info("msdc module reset done 0x10001120: 0x%x, MSDC_CFG: 0x%x\n",
		val, readl(host->base + MSDC_CFG));
}
...
int autok_execute_tuning(struct msdc_host *host, u8 *res)
{
	...
	do {
		...
		if (!ret) {
			...
			msdc_module_reset(host);
			msdc_init_hw(host);
			msdc_set_mclk(host, host->timing, host->mclk);
			retry_cnt--;
		}
	} while (retry_cnt);
